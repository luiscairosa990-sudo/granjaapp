...
# Pool de conexiones para PostgreSQL (más estable en producción)
db_pool = None
...
def init_pool():
    ...
    if not database_url:
        logger.error("=" * 60)
        logger.error("ERROR FATAL: No se encontro DATABASE_URL")
        logger.error("Configurala en Render -> Environment Variables")
        logger.error("=" * 60)
        raise RuntimeError("DATABASE_URL no configurada")
    # Render requiere SSL en PostgreSQL
    db_pool = psycopg2.pool.SimpleConnectionPool(
        1, 10, database_url, sslmode='require', cursor_factory=RealDictCursor
    )
...
def get_factura_numero():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT COUNT(*) as cnt FROM ventas WHERE estado='ACTIVA'")
...
def get_huevos_disponibles():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT COALESCE(SUM(cantidad), 0) as total FROM huevos")
        producidos = c.fetchone()['total'] or 0
        c.execute("SELECT COALESCE(SUM(cantidad), 0) as total FROM ventas WHERE producto_id=1 AND estado='ACTIVA'")
        vendidos = c.fetchone()['total'] or 0
...
def get_pollos_disponibles():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT COALESCE(SUM(cantidad), 0) as total FROM pollos_listos WHERE estado='DISPONIBLE'")
        listos = c.fetchone()['total'] or 0
        c.execute("SELECT COALESCE(SUM(cantidad), 0) as total FROM ventas WHERE producto_id=2 AND estado='ACTIVA'")
        vendidos = c.fetchone()['total'] or 0
...
def get_costos_total(tipo=None):
    ...
    try:
        c = conn.cursor()
        if tipo:
            c.execute("SELECT COALESCE(SUM(valor), 0) as total FROM costos WHERE producto_relacionado=%s", (tipo,))
        else:
            c.execute("SELECT COALESCE(SUM(valor), 0) as total FROM costos")
        return c.fetchone()['total'] or 0
    finally:
        close_db(conn)
...
def get_ventas_total():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT COALESCE(SUM(total), 0) as total FROM ventas WHERE estado='ACTIVA'")
        return c.fetchone()['total'] or 0
    finally:
        close_db(conn)
...
def index():
    ...
    try:
        ...
        alertas = []
        if huevos < 50:
            alertas.append({"tipo": "warning", "msg": f"Pocos huevos disponibles ({huevos} unidades)"})
        if pollos < 10:
            alertas.append({"tipo": "warning", "msg": f"Pocos pollos en inventario ({pollos} unidades)"})
        if gallinas < 20:
            alertas.append({"tipo": "danger", "msg": f"Gallinas por debajo del minimo ({gallinas})"})
        if ganancia < 0 and session.get('rol') == 'admin':
            alertas.append({"tipo": "danger", "msg": f"Perdidas detectadas: ${ganancia:,.2f}"})
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                conn.commit()
                flash("Recoleccion registrada", "success")
            except Exception as e:
                conn.rollback()
                flash(f"Error: {str(e)}", "danger")
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                # Normalizar fecha_pago: string vacio -> None (NULL en PostgreSQL)
                fecha_pago_raw = request.form.get('fecha_pago', '').strip()
                fecha_pago = fecha_pago_raw if fecha_pago_raw else None
...
def factura(id):
    ...
    try:
        ...
        contenido = []
        contenido.append(Paragraph("<b>GRANJA AVICOLA</b>", style_title))
        contenido.append(Paragraph("Sistema de Gestion Integral", styles['Normal']))
        ...
        if venta['telefono']:
            contenido.append(Paragraph(f"Telefono: {venta['telefono']}", styles['Normal']))
        if venta['direccion']:
            contenido.append(Paragraph(f"Direccion: {venta['direccion']}", styles['Normal']))
        contenido.append(Spacer(1, 20))
...
def costos_preparacion(crianza_id):
    ...
    try:
        ...
        data = c.fetchall()
        c.execute("SELECT COALESCE(SUM(valor), 0) as total FROM costos_preparacion WHERE crianza_id=%s", (crianza_id,))
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                c.execute("SELECT COALESCE(SUM(costo), 0) as total FROM alimento WHERE tipo='pollos'")
                costo_alimento = c.fetchone()['total'] or 0
                c.execute("SELECT COALESCE(SUM(valor), 0) as total FROM costos_preparacion WHERE crianza_id=%s", (crianza_id,))
                costo_prep = c.fetchone()['total'] or 0
...
def costos():
    ...
    try:
        ...
        data = c.fetchall()
        c.execute("SELECT categoria, COALESCE(SUM(valor), 0) as total FROM costos GROUP BY categoria")
        resumen = c.fetchall()
...
def reportes():
    ...
    try:
        ...
        c.execute("""
            SELECT categoria, COALESCE(SUM(valor), 0) as total FROM costos
...
def grafica_ventas():
    ...
    try:
        ...
        plt.title("Ventas Ultimos 30 Dias", fontsize=14, fontweight='bold')
        plt.xlabel("Fecha")
        plt.ylabel("Total ($)")
...
def grafica_huevos():
    ...
    try:
        ...
        plt.xticks(rotation=45)
        plt.title("Produccion de Huevos - Ultimos 30 Dias", fontsize=14, fontweight='bold')
        plt.xlabel("Fecha")
        plt.ylabel("Cantidad")
        plt.tight_layout()
        img = BytesIO()
...
# ========================
# RUTA PARA INICIALIZAR DB MANUALMENTE (primera vez)
# ========================
@app.route('/initdb')
def initdb_route():
    """Ejecutar una sola vez despues de crear la base de datos en Render"""
    try:
        init_db()
